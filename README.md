...
项目目前处于 `0.3.0-demo` Steam Demo 候选阶段，已包含派车经营、自有车场访客、场地分成、车辆购买、等级目标、中英双语、紧凑窗口和本地存档闭环。
...
Windows 导出使用 `Windows Demo` 预设。构建输出位于 `builds/windows/`；发布包由该目录中的 `.exe` 与 `.pck` 组成。
