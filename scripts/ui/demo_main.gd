...
func _header() -> Control:
	var row:=HBoxContainer.new();var title:=Label.new();title.text=words.text("app.title");title.add_theme_font_size_override("font_size",24);title.size_flags_horizontal=Control.SIZE_EXPAND_FILL;row.add_child(title)
	var next_xp:=Config.LEVEL_XP[state.level] if state.level<5 else Config.LEVEL_XP[-1]
	var stats:=Label.new();stats.text=words.text("stats",[state.coins,state.level,state.xp,next_xp]);row.add_child(stats)
	for key in ["language","window"]:
		var button:=Button.new();button.text=words.text("action.language") if key=="language" else words.text("action.normal" if state.compact else "action.compact")
		button.pressed.connect(_toggle_language if key=="language" else _toggle_window);row.add_child(button)
...
func _tutorial() -> Control:
	...
	var label:=Label.new();label.text=words.text("tutorial.%d"%state.tutorial_step);label.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;panel.add_child(label);return panel
...
func _car_row(car: Dictionary) -> Control:
	var row:=HBoxContainer.new();row.custom_minimum_size.y=58;row.add_theme_constant_override("separation",10);var color:=ColorRect.new();color.color=Color(car.color);color.custom_minimum_size=Vector2(52,46);row.add_child(color)
	...
	row.add_child(text);var action:=Button.new()
...
func _home_page() -> Control:
	var box:=VBoxContainer.new();var image:=TextureRect.new();image.texture=BACKGROUND;image.expand_mode=TextureRect.EXPAND_IGNORE_SIZE;image.stretch_mode=TextureRect.STRETCH_KEEP_ASPECT_COVERED;image.custom_minimum_size=Vector2(300,190 if state.compact else 260);box.add_child(image)
